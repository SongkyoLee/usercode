...
//	  	raw_inclusive[ic][ip]->SetMaximum(1.2);
  		raw_inclusive[ic][ip]->SetMinimum(300);
	  	raw_inclusive[ic][ip]->SetMaximum(3900);
...
//			text->AddText(Form("y_{cm} : %s",rapArr[ic].c_str()));
			...
			text->Draw();
			c1->Update();
			c1->SaveAs(Form("rawYield_yDep_pPb_finer2_%s_%s.png",centArr[ic].c_str(),ptArr[ip].c_str()));
			leg->Clear();
			text->Clear();
			c1->Clear();
		}
