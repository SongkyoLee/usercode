...
//	string centArr[] = {"70-100", "60-70", "50-60", "40-50", "30-40", "20-30", "10-20", "0-10"};
	...
	// ----Background info from fit_parameter
...
//			cout << " Pbp Raw=" <<setw(10) << nPbpSig[ic][ip][ir] << setw(10) << nPbpSigErr[ic][ip][ir] << setw(10)<< nPbpPr[ic][ip][ir] << setw(10) << nPbpPrErr[ic][ip][ir] << setw(10) <<  nPbpNp[ic][ip][ir] << setw(10) << nPbpNpErr[ic][ip][ir] << setw(10) << endl;
//			cout << " pPb Raw="<<setw(10) << npPbSig[ic][ip][ir] << setw(10) << npPbSigErr[ic][ip][ir] << setw(10)<< npPbPr[ic][ip][ir] << setw(10) << npPbPrErr[ic][ip][ir] << setw(10) << npPbNp[ic][ip][ir] << setw(10) << npPbNpErr[ic][ip][ir] << setw(10) << endl;
...
//	TLegend *leg = new TLegend(0.68, 0.74, 0.85, 0.90); //upper right
	TLegend *leg = new TLegend(0.23, 0.75, 0.40, 0.90); //upper left
...
//	TPaveText *text = new TPaveText(0.20, 0.48, 0.50, 0.64, "NDC"); //upper left
	TPaveText *text = new TPaveText(0.68, 0.75, 0.85, 0.90, "NDC"); //upper left
...
//	TPaveText *text = new TPaveText(0.20, 0.58, 0.40, 0.72, "NDC"); //middle left
	...
	for (Int_t ic=0; ic<nCent; ic++) {
		for (Int_t ip=0; ip<nPt; ip++) {
			raw_inclusive[ic][ip] = new TGraphErrors(nRap, xaxis, nSigCorr1[ic][ip], 0, nSigCorr1Err[ic][ip]);
			...
			raw_inclusive[ic][ip]->GetYaxis()->SetTitle("N_{J/#psi}/A");
  		raw_inclusive[ic][ip]->SetMinimum(0.);
	  	raw_inclusive[ic][ip]->SetMaximum(35000.);
			raw_inclusive[ic][ip]->Draw("ALP");
			raw_prompt[ic][ip] = new TGraphErrors(nRap, xaxis, nPrCorr1[ic][ip], 0, nPrCorr1Err[ic][ip]);
			SetGraphStyle(raw_prompt[ic][ip], 0, 0);
			raw_prompt[ic][ip]->Draw("LP");
			raw_nonprompt[ic][ip] = new TGraphErrors(nRap, xaxis, nNpCorr1[ic][ip], 0, nNpCorr1Err[ic][ip]);
...
//			text->AddText(Form("y_{cm} : %s",rapArr[ic].c_str()));
			...
			text->Draw();
			c1->Update();
			c1->SaveAs(Form("rawAccep_yDep_finer2_%s_%s.pdf",centArr[ic].c_str(),ptArr[ip].c_str()));
			leg->Clear();
			text->Clear();
			c1->Clear();
		}
