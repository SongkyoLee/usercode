...
for f in range(2,len(sys.argv)):
  ...
  for files in filelist:  # 1 bin per a line
    ...
    thisEntryHas = rapN+" "+ptN+" "+centN+" "+dphiN
    if [ check for check in duplicateCheck if str(thisEntryHas) in str(check) ]:
      ...
      duplicateCheck.append(rapN+" "+ptN+" "+centN+" "+dphiN)
      datapar.append(data)
  ...
  ferror.write("rap\tpT\tcent\tdPhi\tNSig\tNSigErr\tsigWidth\tsigWidthErr\n");

  for i in dataparfin:
    # Check inclusive yields and if it is too large, write those bins into a error file
    if i[8]*0.15 < i[9] or ((i[32]*0.2 < i[33]) and (i[33] > 0.01)) :
      ferror.write(str(i[0])+"\t"+str(i[1])+"\t"+str(i[2])+"\t"+str(i[3])+"\t")
      ferror.write(str(i[8])+"\t"+str(i[9])+"\t")
      ferror.write(str(i[32])+"\t"+str(i[33])+"\n")

  # Ctau error range will be wrintten
  if f is 2:
    fctauerr.write("rap\tpT\tcent\tdPhi\tctauErrMin\tctauErrMax\n")
  datactauerrfin = sorted(datactauerr, key=itemgetter(0,1,2,3))
  ...
  for i in datatabfin:
    headerbin = "rap" + i[0] + "_pT" + i[1] + "_cent" + i[2] + "\n";
