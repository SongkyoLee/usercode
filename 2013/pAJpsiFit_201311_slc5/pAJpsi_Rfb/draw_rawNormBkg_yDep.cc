...
//	string centArr[] = {"70-100", "60-70", "50-60", "40-50", "30-40", "20-30", "10-20", "0-10"};
	...
	// normalized = raw yield / Bkg yield
	Double_t nSigNorm[nCent][nPt][nRap], nPrNorm[nCent][nPt][nRap], nNpNorm[nCent][nPt][nRap];
	Double_t nSigNormErr[nCent][nPt][nRap], nPrNormErr[nCent][nPt][nRap], nNpNormErr[nCent][nPt][nRap];
...
for (Int_t ic=0; ic<nCent; ic++) {
	for (Int_t ip=0; ip<nPt; ip++) { 
		for (Int_t ir=0; ir<nRap; ir++) {
			...
			// yield normalization and error propagation
			nSigNorm[ic][ip][ir] = nSig[ic][ip][ir]/nBkg[ic][ip][ir];
			nPrNorm[ic][ip][ir] = nPr[ic][ip][ir]/nBkg[ic][ip][ir];
			nNpNorm[ic][ip][ir] = nNp[ic][ip][ir]/nBkg[ic][ip][ir];
			nSigNormErr[ic][ip][ir]= nSigNorm[ic][ip][ir]*TMath::Sqrt( TMath::Power(nSigErr[ic][ip][ir]/nSig[ic][ip][ir],2)+TMath::Power(nBkgErr[ic][ip][ir]/nBkg[ic][ip][ir],2) );
			nPrNormErr[ic][ip][ir]= nPrNorm[ic][ip][ir]*TMath::Sqrt( TMath::Power(nPrErr[ic][ip][ir]/nPr[ic][ip][ir],2)+TMath::Power(nBkgErr[ic][ip][ir]/nBkg[ic][ip][ir],2) );
			nNpNormErr[ic][ip][ir]= nNpNorm[ic][ip][ir]*TMath::Sqrt( TMath::Power(nNpErr[ic][ip][ir]/nNp[ic][ip][ir],2)+TMath::Power(nBkgErr[ic][ip][ir]/nBkg[ic][ip][ir],2) );
		}
	}
}
...
//	TPaveText *text = new TPaveText(0.20, 0.58, 0.40, 0.72, "NDC"); //middle left
	...
	for (Int_t ic=0; ic<nCent; ic++) {
		for (Int_t ip=0; ip<nPt; ip++) {
			raw_inclusive[ic][ip] = new TGraphErrors(nRap, xaxis, nSigNorm[ic][ip], 0, nSigNormErr[ic][ip]);
			...
			raw_inclusive[ic][ip]->GetYaxis()->SetTitle("raw yield / bkg yield");
  		raw_inclusive[ic][ip]->SetMinimum(0.);
	  	raw_inclusive[ic][ip]->SetMaximum(11.);
			raw_inclusive[ic][ip]->Draw("ALP");
			raw_prompt[ic][ip] = new TGraphErrors(nRap, xaxis, nPrNorm[ic][ip], 0, nPrNormErr[ic][ip]);
			SetGraphStyle(raw_prompt[ic][ip], 0, 0);
			raw_prompt[ic][ip]->Draw("LP");
			raw_nonprompt[ic][ip] = new TGraphErrors(nRap, xaxis, nNpNorm[ic][ip], 0, nNpNormErr[ic][ip]);
...
//			text->AddText(Form("y_{cm} : %s",rapArr[ic].c_str()));
			...
			text->Draw();
			c1->Update();
			c1->SaveAs(Form("rawNormBkg_yDep_finer2_%s_%s.png",centArr[ic].c_str(),ptArr[ip].c_str()));
			leg->Clear();
			text->Clear();
			c1->Clear();
		}
