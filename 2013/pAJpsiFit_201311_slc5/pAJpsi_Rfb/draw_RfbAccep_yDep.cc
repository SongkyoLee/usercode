...
void draw_RfbAccep_yDep()
{
	...
	std::ifstream inFileAcc("acceptance_yDep",std::ios::in);
...
//	string centArr[] = {"70-100", "60-70", "50-60", "40-50", "30-40", "20-30", "10-20", "0-10"};
	...
	// get acceptance
	Double_t nAcc[nCent][nPt][nRap];
	Double_t nAccErr[nCent][nPt][nRap];
	// corrected yield 1 = raw yield / Acceptance
	Double_t nSigCorr1[nCent][nPt][nRap], nPrCorr1[nCent][nPt][nRap], nNpCorr1[nCent][nPt][nRap];
	Double_t nSigCorr1Err[nCent][nPt][nRap], nPrCorr1Err[nCent][nPt][nRap], nNpCorr1Err[nCent][nPt][nRap];
	// Rfb 
...
for (Int_t ic=0; ic<nCent; ic++) {
	for (Int_t ip=0; ip<nPt; ip++) { 
		for (Int_t ir=0; ir<nRap; ir++) {
			...
			inFileAcc >> nAcc[ic][ip][ir] >> nAccErr[ic][ip][ir];
			// merge Pbp & pPb data
...
//			cout << " Total Raw="<<setw(10) << nSig[ic][ip][ir] << setw(10) << nSigErr[ic][ip][ir] << setw(10)<< nPr[ic][ip][ir] << setw(10) << nPrErr[ic][ip][ir] << setw(10) <<  nNp[ic][ip][ir] << setw(10) << nNpErr[ic][ip][ir] << setw(10) << nAcc[ic][ip][ir] << setw(10) << nAccErr[ic][ip][ir] << endl;
			// yield correction 1
			nSigCorr1[ic][ip][ir] = nSig[ic][ip][ir]/nAcc[ic][ip][ir];
			nPrCorr1[ic][ip][ir] = nPr[ic][ip][ir]/nAcc[ic][ip][ir];
			nNpCorr1[ic][ip][ir] = nNp[ic][ip][ir]/nAcc[ic][ip][ir];
			nSigCorr1Err[ic][ip][ir]= nSigCorr1[ic][ip][ir]*TMath::Sqrt( TMath::Power(nSigErr[ic][ip][ir]/nSig[ic][ip][ir],2)+TMath::Power(nAccErr[ic][ip][ir]/nAcc[ic][ip][ir],2) );
			nPrCorr1Err[ic][ip][ir]= nPrCorr1[ic][ip][ir]*TMath::Sqrt( TMath::Power(nPrErr[ic][ip][ir]/nPr[ic][ip][ir],2)+TMath::Power(nAccErr[ic][ip][ir]/nAcc[ic][ip][ir],2) );
			nNpCorr1Err[ic][ip][ir]= nNpCorr1[ic][ip][ir]*TMath::Sqrt( TMath::Power(nNpErr[ic][ip][ir]/nNp[ic][ip][ir],2)+TMath::Power(nAccErr[ic][ip][ir]/nAcc[ic][ip][ir],2) );
			cout << " N_{J/#psi}/A = " <<setw(10) << nSigCorr1[ic][ip][ir] << setw(10) << nSigCorr1Err[ic][ip][ir]     << setw(10)<< nPrCorr1[ic][ip][ir] << setw(10) << nPrCorr1Err[ic][ip][ir] << setw(10) <<  nNpCorr1[ic][ip][    ir] << setw(10) << nNpCorr1Err[ic][ip][ir] << setw(10) << endl;
		}
	}
}
...
for (Int_t ic=0; ic<nCent; ic++) {
	for (Int_t ip=0; ip<nPt; ip++) { 
		for (Int_t ir=0; ir<nXaxis; ir++) {
			rfbSig[ic][ip][ir] = nSigCorr1[ic][ip][ir]/nSigCorr1[ic][ip][nRap-1-ir]; // Fw/Bw
			rfbPr[ic][ip][ir] = nPrCorr1[ic][ip][ir]/nPrCorr1[ic][ip][nRap-1-ir];
			rfbNp[ic][ip][ir] = nNpCorr1[ic][ip][ir]/nNpCorr1[ic][ip][nRap-1-ir];
			rfbSigErr[ic][ip][ir]= rfbSig[ic][ip][ir]*TMath::Sqrt( TMath::Power(nSigCorr1Err[ic][ip][ir]/nSigCorr1[ic][ip][ir],2)+TMath::Power(nSigCorr1Err[ic][ip][nRap-1-ir]/nSigCorr1[ic][ip][nRap-1-ir],2) );
			rfbPrErr[ic][ip][ir]= rfbPr[ic][ip][ir]*TMath::Sqrt( TMath::Power(nPrCorr1Err[ic][ip][ir]/nPrCorr1[ic][ip][ir],2)+TMath::Power(nPrCorr1Err[ic][ip][nRap-1-ir]/nPrCorr1[ic][ip][nRap-1-ir],2) );
			rfbNpErr[ic][ip][ir]= rfbNp[ic][ip][ir]*TMath::Sqrt( TMath::Power(nNpCorr1Err[ic][ip][ir]/nNpCorr1[ic][ip][ir],2)+TMath::Power(nNpCorr1Err[ic][ip][nRap-1-ir]/nNpCorr1[ic][ip][nRap-1-ir],2) );
...
//	TPaveText *text = new TPaveText(0.20, 0.48, 0.50, 0.64, "NDC"); //upper left
	...
	for (Int_t ic=0; ic<nCent; ic++) {
		for (Int_t ip=0; ip<nPt; ip++) {
			...
			SetGraphStyle(rfb_inclusive[ic][ip], 7, 0);
			rfb_inclusive[ic][ip]->SetTitle("");
			rfb_inclusive[ic][ip]->GetXaxis()->SetTitle("y_{cm}");
			rfb_inclusive[ic][ip]->GetXaxis()->CenterTitle();
			rfb_inclusive[ic][ip]->GetYaxis()->SetTitle("R_{FB} (A corr)");
			rfb_inclusive[ic][ip]->GetXaxis()->SetRangeUser(0.,1.93);
...
//			c1->SaveAs(Form("Rfb_yDep_finer_%s_%s.pdf",centArr[ic].c_str(),ptArr[ip].c_str()));
			c1->SaveAs(Form("RfbAccep_yDep_finer2_%s_%s.pdf",centArr[ic].c_str(),ptArr[ip].c_str()));
			leg->Clear();
			text->Clear();
			c1->Clear();
		}
