...
void draw_centDependency_pPb_Pbp_merged()
{ 
...
//	static const double Jpsi_CtMax = 3.0;
	...
	string histTitle[]={
		"Jpsi [2.95,3.25] MeV/c^{2}, y_cm<1.93;Centrality;Counts", 
		"psi2s [3.53,3.83] MeV/c^{2}, y_cm<1.93;Centrality;Counts", 
		"Jpsi [2.95,3.25] MeV/c^{2}, y_cm<1.93;N_{tracks};Counts", 
		"psi2s [3.53,3.83] MeV/c^{2}, y_cm<1.93;N_{tracks};Counts", 
		"Jpsi [2.95,3.25] MeV/c^{2}, y_cm<1.93;E_{T}^{HF |#eta|>4};Counts", 
		"psi2s [3.53,3.83] MeV/c^{2}, y_cm<1.93;E_{T}^{HF |#eta|>4};Counts", 
	};
	...
	// [2] : J/psi dbleMu&singluMu -1.47< y_lab(eta_lab) <2.4 for pPb ( |y_cm| <1.93 )
	...
	//int nBin[] = {30}; 
	const int nBin[]= {15, 15, 30, 30, 30, 30}; 
	const int nMin[] = {0, 0, 0, 0, 0, 0};
	const int nMax[] = {100, 100, 300, 300, 90, 90};
	...
	for (Int_t i=0; i<nHist; i++) {
		histCent[i] = new TH1D(histName[i].c_str(), histTitle[i].c_str(), nBin[i], nMin[i], nMax[i]);
		histCent[i]->Sumw2();
//		colorIndex=1;
...
//		histCent[i]->SetMaximum(10000.0);
	...
	TPaveText *text = new TPaveText(0.34, 0.15, 0.50, 0.30, "NDC"); // bottom left small
	...
	// 1.pbp_ppb_cent
	treePbp->Draw("Centrality>>pbp_ppb_Jpsi_cent", condition[0].c_str(),"e");
	treepPb->Draw("Centrality>>+pbp_ppb_Jpsi_cent", condition[2].c_str(),"e");
	treePbp->Draw("Centrality>>pbp_ppb_psi2s_cent", condition[1].c_str(),"esame");
	treepPb->Draw("Centrality>>+pbp_ppb_psi2s_cent", condition[3].c_str(),"esame");
...
//	text->AddText("[2.95-3.25] GeV/c^{2}");
	...
	// 2.pbp_ppb_Ntrk
	treePbp->Draw("Ntracks>>pbp_ppb_Jpsi_Ntrk", condition[0].c_str(),"e");
	treepPb->Draw("Ntracks>>+pbp_ppb_Jpsi_Ntrk", condition[2].c_str(),"e");
	treePbp->Draw("Ntracks>>pbp_ppb_psi2s_Ntrk", condition[1].c_str(),"esame");
	treepPb->Draw("Ntracks>>+pbp_ppb_psi2s_Ntrk", condition[3].c_str(),"esame");
	...
	// 3.pbp_ppb_EtHF
//	treePbp->Draw("SumET_HF>>pbp_Jpsi_EtHF", condition[0].c_str(),"e");
//	treePbp->Draw("SumET_HF>>pbp_psi2s_EtHF", condition[1].c_str(),"esame");
	treePbp->Draw("SumET_HFplusEta4+SumET_HFminusEta4>>pbp_ppb_Jpsi_EtHF", condition[0].c_str(),"e");
	treepPb->Draw("SumET_HFplusEta4+SumET_HFminusEta4>>+pbp_ppb_Jpsi_EtHF", condition[2].c_str(),"e");
	treePbp->Draw("SumET_HFplusEta4+SumET_HFminusEta4>>pbp_ppb_psi2s_EtHF", condition[1].c_str(),"esame");
	treepPb->Draw("SumET_HFplusEta4+SumET_HFminusEta4>>+pbp_ppb_psi2s_EtHF", condition[3].c_str(),"esame");
