    ...
    if (inOpt.isPbPb == 1) { 
      if (!inOpt.yrange.compare("0.0-2.4")) {
        if (!inOpt.prange.compare("7.5-8.5")) {
          ws->var("alpha")->setRange(1,5);
          ws->var("enneW")->setRange(0,100);
          sprintf(funct,"SUM::sigMassPDF(NSig[%f,1.0,50000.0]*%s,NBkg[%f,1.0,500000.0]*%s)",initSig,initBkg,inOpt.mSigFunct.c_str(),inOpt.mBkgFunct.c_str());
        }
      } // rap0.0-2.4
      else if (!inOpt.yrange.compare("2.1-2.4")) {
        if (!inOpt.prange.compare("6.5-30.0")) {
          ws->var("alpha")->setRange(0,5);
          ws->var("enneW")->setRange(1,100);
          ws->var("sigmaSig1")->setRange(0.01,0.22);
      ...
      if (!inOpt.yrange.compare("0.0-2.4")) {
        if (!inOpt.prange.compare("9.5-10.5")) {
          ws->var("alpha")->setRange(1,5);
          ws->var("enneW")->setRange(2,100);
          sprintf(funct,"SUM::sigMassPDF(NSig[%f,1.0,50000.0]*%s,NBkg[%f,1.0,500000.0]*%s)",initSig,initBkg,inOpt.mSigFunct.c_str(),inOpt.mBkgFunct.c_str());
        }
        else if (!inOpt.prange.compare("11.5-12.5")) {
          ws->var("alpha")->setRange(1,5);
          ws->var("enneW")->setRange(2,100);
          sprintf(funct,"SUM::sigMassPDF(NSig[%f,1.0,50000.0]*%s,NBkg[%f,1.0,500000.0]*%s)",initSig,initBkg,inOpt.mSigFunct.c_str(),inOpt.mBkgFunct.c_str());
        }
        else if (!inOpt.prange.compare("20.0-30.0")) {
          ws->var("alpha")->setRange(1,5);
          ws->var("enneW")->setRange(2,100);
          sprintf(funct,"SUM::sigMassPDF(NSig[%f,1.0,50000.0]*%s,NBkg[%f,1.0,500000.0]*%s)",initSig,initBkg,inOpt.mSigFunct.c_str(),inOpt.mBkgFunct.c_str());
        }
...
void formRapidity(InputOpt &opt, float ymin, float ymax) {
  ...
  if (!opt.rpmethod.compare("etHFm")) {
    if (fracMin==0 && fracMax!=0) sprintf(opt.rapString,"%.0f < y < %.2f",ymin,ymax);
    else if (fracMin!=0 && fracMax==0) sprintf(opt.rapString,"%.2f < y < %.0f",ymin,ymax);
    else if (fracMin==0 && fracMax==0) sprintf(opt.rapString,"%.0f < y < %.0f",ymin,ymax);
    else sprintf(opt.rapString,"%.2f < y < %.2f",ymin,ymax);
  } else if (!opt.rpmethod.compare("etHFp")) {
    if (fracMin!=0 && fracMax==0) sprintf(opt.rapString,"%.0f < y < %.2f",-ymax,-ymin);
    else if (fracMin==0 && fracMax!=0) sprintf(opt.rapString,"%.2f < y < %.0f",-ymax,-ymin);
    else if (fracMin==0 && fracMax==0) sprintf(opt.rapString,"%.0f < y < %.0f",-ymax,-ymin);
    else sprintf(opt.rapString,"%0.2f < y < %.2f",-ymax,-ymin);
  } else {
    if (ymin==0 && fracMax!=0) sprintf(opt.rapString,"|y| < %.2f",ymax);
    else if (fracMin==0 && fracMax!=0) sprintf(opt.rapString,"%.0f < |y| < %.2f",ymin,ymax);
    else if (fracMin==0 && fracMax==0) sprintf(opt.rapString,"%.0f < |y| < %.0f",ymin,ymax);
    else if (fracMin!=0 && fracMax==0) sprintf(opt.rapString,"%.2f < |y| < %.0f",ymin,ymax);
    else sprintf(opt.rapString,"%.2f < |y| < %.2f",ymin,ymax);
  }
...
//          cout << "getCtauErrRange:: BKG upper edge: " << hBkg->GetBinLowEdge(xbins+1) << endl;
  ...
  if (opt.isPbPb != 0) t->DrawLatex(0.50,0.60,opt.centString);
...
void sidebandLeftRightCheck(RooWorkspace *ws, RooDataSet *redDataSBL, RooDataSet *redDataSBR, InputOpt &opt) {
  ...
  c0.SaveAs(titlestr.c_str());
  titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_testCtauSB_Lin.root";
  c0.SaveAs(titlestr.c_str());
  c0.SetLogy(1);
  ...
  c0.SaveAs(titlestr.c_str()); 
  titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_testCtauSB_Log.root";
  c0.SaveAs(titlestr.c_str()); 
}
...
/*    ws->var("Jpsi_Mass")->setRange("sigpeak",2.9,3.3);
  ...
  titlestr = opt.dirPre + "_rap" + opt.yrange  + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_massfit_wob_wopull.root";
  cmwop.SaveAs(titlestr.c_str());
  ...
  titlestr = opt.dirPre + "_rap" + opt.yrange  + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_massfit_wob.root";
  cm.SaveAs(titlestr.c_str());
...
/*  c0.Clear(); c0.cd(); c0.SetLogy(0); errframe->Draw();
  ...
  c0.SaveAs(titlestr.c_str());
  titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_testErrPdfBkg_Log.root";
  c0.SaveAs(titlestr.c_str());
  delete errframe;
...
/*   c0.cd(); c0.Clear(); c0.SetLogy(1); errframe->Draw();
  ...
  c0.SaveAs(titlestr.c_str());
  titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_testErrPdfSig_Log.root";
  c0.SaveAs(titlestr.c_str());
  delete errframe;
...
void ctauResolFitCheck(RooWorkspace *ws, bool fitMC, RooPlot *tframePR, InputOpt &opt) {
  ...
  if (fitMC)
    titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_resolInit_Lin.root";
  else
    titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_resolFit_Lin.root";
  c00.SaveAs(titlestr.c_str());
  ...
  c00.SaveAs(titlestr.c_str());
  if (fitMC)
    titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_resolInit_Log.root";
  else
...
void drawCtauSBPlots(RooWorkspace *ws, RooDataSet *redDataSB, RooDataSet *redDataSBL, RooDataSet *redDataSBR, RooDataHist *binDataCtErrSB, RooFitResult *fitSB, RooFitResult *fitSBL, RooFitResult *fitSBR, InputOpt &opt) {
  ...
  if (opt.ctauBackground == 0) {
    ...
    titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_timeside_Lin.root";
    c3->SaveAs(titlestr.c_str());
    ...
    titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_timeside_Log.root";
    c3a->SaveAs(titlestr.c_str());
    ...
    c3->SaveAs(titlestr.c_str());
    titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_timesideL_Lin.root";
    c3->SaveAs(titlestr.c_str());
    ...
    c3a->SaveAs(titlestr.c_str());
    titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_timesideL_Log.root";
    c3a->SaveAs(titlestr.c_str());
    ...
    c3->SaveAs(titlestr.c_str());
    titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_timesideR_Lin.root";
    c3->SaveAs(titlestr.c_str());
    ...
    c3a->SaveAs(titlestr.c_str());
    titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_timesideR_Log.root";
    c3a->SaveAs(titlestr.c_str());
...
void drawMassPlotsWithB(RooWorkspace *ws, RooDataSet* redDataCut, float NSigNP_fin, float NBkg_fin, RooFitResult *fitM, InputOpt &opt) {
  ...
  titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_massfit_wopull.root";
  c1wop.SaveAs(titlestr.c_str());
  ...
  titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_massfit.root";
  c1.SaveAs(titlestr.c_str());
}
...
//  TLegend * leg = new TLegend(0.455,0.35,0.85,0.54,NULL,"brNDC");   //with dPhistring
  ...
  titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_timefit_Lin.root";
  c2->SaveAs(titlestr.c_str());
...
//  leg->SetY2NDC(0.57);
  ...
  titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange +  "_cent" + opt.crange + "_dPhi" + opt.phirange + "_timefit_Log.root";
  c2a->SaveAs(titlestr.c_str());
  delete c2a;
...
//  h2D->GetZaxis()->SetTitle(Form("Counts / %.2f (mm)",avgBinWidth));
  ...
  titlestr = opt.dirPre + "_rap" + opt.yrange + "_pT" + opt.prange + "_cent" + opt.crange + "_dPhi" + opt.phirange + "_2D_totFit.root";
  c2c->SaveAs(titlestr.c_str());
  delete c2c;
}
