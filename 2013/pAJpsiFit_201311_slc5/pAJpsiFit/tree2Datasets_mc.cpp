...
//0: default 1: flip the rapidity signs
...
int main(int argc, char* argv[]) {
  ...
  if (argc == 1) {
    ...
    cout << "./Tree2Datasets =op [RP number] =ot [trigType] =or [runType] =od [dataType] =w [Weighting] =f [input TTree file] [output directory]" << endl;
    cout << "./Tree2Datasets =op -1 =ot 3 =or 0 =od 0 =w 0 =f /tmp/miheejo/mini_Jpsi_Histos_may202012_m25gev.root ." << endl;
    cout << "====================================================================\n";
    return 0;
  }
...
*/
  ...
  // Without weighting for MC
  RooArgList varlist(*Jpsi_Mass,*Jpsi_Pt,*Jpsi_Y,*Jpsi_dPhi,*Jpsi_Cent,*Jpsi_Ct,*Jpsi_CtErr,*MCType,*Jpsi_CtTrue);
  RooArgList varlistSame(*Jpsi_Mass,*Jpsi_Pt,*Jpsi_Y,*Jpsi_dPhi,*Jpsi_Cent,*Jpsi_Ct,*Jpsi_CtErr,*MCType,*Jpsi_CtTrue);
  RooArgList varlist2(*Psip_Mass,*Jpsi_Pt,*Jpsi_Y,*Jpsi_dPhi,*Jpsi_Cent,*Jpsi_Ct,*Jpsi_CtErr,*MCType,*Jpsi_CtTrue);
  // With weighting for MC
...
*/
  ...
  for (int ev=initev; ev<nevt; ++ev) {
    ...
    for (int i=0; i<Reco_QQ_size; ++i) {
      ...
      Jpsi.theMass =JP->M();
			if(dataType == 1) Jpsi.theRapidity=(-1)*JP->Rapidity(); // for Pbp (MC)
			else Jpsi.theRapidity=JP->Rapidity(); //for pPb, pp
			//cout<<"dataType="<<dataType<<", corrected Y="<<Jpsi.theRapidity<<", original Y="<<JP->Rapidity()<<endl;
      Jpsi.thePt=JP->Pt();
      Jpsi.thePhi = JP->Phi();
...
*/
  ...
  /*        RooArgList varlist_tmp(*Jpsi_Mass,*Jpsi_Pt,*Jpsi_Y,*Jpsi_Type,*Jpsi_Sign,*Jpsi_Ct,*Jpsi_CtErr);
          ...
          // Without weighting
          RooArgList varlist_tmp(*Jpsi_Mass,*Jpsi_Pt,*Jpsi_Y,*Jpsi_dPhi,*Jpsi_Cent,*Jpsi_Ct,*Jpsi_CtErr,*MCType,*Jpsi_CtTrue);
          RooArgList varlistSame_tmp(*Jpsi_Mass,*Jpsi_Pt,*Jpsi_Y,*Jpsi_dPhi,*Jpsi_Cent,*Jpsi_Ct,*Jpsi_CtErr,*MCType,*Jpsi_CtTrue);
          RooArgList varlist2_tmp(*Psip_Mass,*Jpsi_Pt,*Jpsi_Y,*Jpsi_dPhi,*Jpsi_Cent,*Jpsi_Ct,*Jpsi_CtErr,*MCType,*Jpsi_CtTrue);
          // With weighting
