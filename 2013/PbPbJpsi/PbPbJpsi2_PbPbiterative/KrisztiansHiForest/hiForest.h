...
class HiForest : public TNamed
{
  ...
  TTree *pfTowerTree;                           // pfTower Tree K.K.
...
//  if (collisionMode == cPbPb || collisionMode == cPP) trackTree        = (TTree*) inf->Get("mergedTrack/trackTree");
  if (collisionMode == cPPb) trackTree        = (TTree*) inf->Get("ppTrack/trackTree");
  towerTree        = (TTree*) inf->Get("rechitanalyzer/tower");
  pfTowerTree      = (TTree*) inf->Get("pfTowers/hf");
  ...
  if (hasPfTowerTree) {
    pfTowerTree->SetName("hf");
    if (tree == 0) tree = pfTowerTree; else tree->AddFriend(pfTowerTree);
    setupHitTree(pfTowerTree,hf);
  }
...
void HiForest::GetEntry(int i)
{
  ...
  if (hasPfTowerTree)    pfTowerTree    ->GetEntry(i);
...
//            trackCorrections[i]->AddNormFile("trkcorr/IterTrkCorrv12XSec/IterTrkCorrv12XSec_hy18dj200_akPu3PF_-1_-1_-1000_genJetMode0.root");
         //KK
           ...
           trackCorrections[i]->AddSample("trkcorr/EricCorrections/Pythia_ridgeSel_corrv0_akPu3PF_-1_-1_-1000_genJetMode0.root",0);
         } else {
         ...
         trackCorrections[i]->ppMode_ = (collisionMode==cPP || collisionMode==cPPb); //K.K.
...
void HiForest::PrintStatus()
{
  ...
  if (hasPfTowerTree)    CheckTree(pfTowerTree,    "PfTowerTree");
...
void HiForest::SetOutputFile(const char *name)
{
  ...
  if (hasPfTowerTree)    AddCloneTree(pfTowerTree,    "pfTowers",              "hf");
