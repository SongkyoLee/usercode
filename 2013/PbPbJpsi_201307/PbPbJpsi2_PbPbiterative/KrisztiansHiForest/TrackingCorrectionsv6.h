...
//             hnameNoE = Form("hPtHatBeforeSel");
   ...
   cout << " ptHatMin_.size(): " << ptHatMin_.size() << endl;
   cout << " corrSetName_: " << corrSetName_ << endl;
   for (UInt_t s=0; s<ptHatMin_.size(); ++s) { // merge pt hat samples with weight
     ...
     if (ppMode_) {
        ...
        else if ( ptHatMin_[s]==0) sampleCroSec_[s] = 1e-9; //I copied basePtHat
        else cout << endl << endl << " Error no such pt hat!!!!!" << endl << endl << endl;
     }
     float basePtHat;
...
//   else {
//       cout << " meaningful correction " << endl; //K.K.
//   }
