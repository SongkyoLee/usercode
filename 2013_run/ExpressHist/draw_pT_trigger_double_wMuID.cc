...
void draw_pT_trigger_double_wMuID()
{
	//Set style
	...
	string histName[] ={
		"All_Global_muons",
		"L1DoubleMuOpen_v1", 
		"L1DoubleMu0_HighQ_v1", 
		"L2DoubleMu3_v1", 
		"PxlTrkMulti100_L2DoubleMu3_v1"
	};
	...
	tree->Draw("HLTMuTree.Glb_pt>>L1DoubleMuOpen_v1","HLTMuTree.Event==HltTree.Event && HLTMuTree.Glb_nptl>0 && HLTMuTree.Glb_nTrkFound>10 && HLTMuTree.Glb_glbChi2_ndof<20. && HLTMuTree.Glb_trkChi2_ndof<4. && HLTMuTree.Glb_pixLayerWMeas>0 && TMath::Abs(HLTMuTree.Glb_trkDxy)<3. && TMath::Abs(HLTMuTree.Glb_trkDz)<15. && HltTree.HLT_PAL1DoubleMuOpen_v1==1","pe same");
	tree->Draw("HLTMuTree.Glb_pt>>L1DoubleMu0_HighQ_v1","HLTMuTree.Event==HltTree.Event && HLTMuTree.Glb_nptl>0 && HLTMuTree.Glb_nTrkFound>10 && HLTMuTree.Glb_glbChi2_ndof<20. && HLTMuTree.Glb_trkChi2_ndof<4. && HLTMuTree.Glb_pixLayerWMeas>0 && TMath::Abs(HLTMuTree.Glb_trkDxy)<3. && TMath::Abs(HLTMuTree.Glb_trkDz)<15. && HltTree.HLT_PAL1DoubleMu0_HighQ_v1==1","pe same");
	tree->Draw("HLTMuTree.Glb_pt>>L2DoubleMu3_v1","HLTMuTree.Event==HltTree.Event && HLTMuTree.Glb_nptl>0 && HLTMuTree.Glb_nTrkFound>10 && HLTMuTree.Glb_glbChi2_ndof<20. && HLTMuTree.Glb_trkChi2_ndof<4. && HLTMuTree.Glb_pixLayerWMeas>0 && TMath::Abs(HLTMuTree.Glb_trkDxy)<3. && TMath::Abs(HLTMuTree.Glb_trkDz)<15. && HltTree.HLT_PAL2DoubleMu3_v1==1","pe same");
	tree->Draw("HLTMuTree.Glb_pt>>PxlTrkMulti100_L2DoubleMu3_v1","HLTMuTree.Event==HltTree.Event && HLTMuTree.Glb_nptl>0 && HLTMuTree.Glb_nTrkFound>10 && HLTMuTree.Glb_glbChi2_ndof<20. && HLTMuTree.Glb_trkChi2_ndof<4. && HLTMuTree.Glb_pixLayerWMeas>0 && TMath::Abs(HLTMuTree.Glb_trkDxy)<3. && TMath::Abs(HLTMuTree.Glb_trkDz)<15. && HltTree.HLT_PAPixelTrackMultiplicity100_L2DoubleMu3_v1==1","pe same");

	c1->SaveAs(Form("trig_double_cut_run%d.pdf",Run));

	//save as a root file
	TFile *outFile = new TFile(Form("trig_double_cut_run%d.root",Run),"recreate");
	outFile->cd();
	for (Int_t i=0; i<nHist; i++) {
		hist1D[i]->Write();
	}
