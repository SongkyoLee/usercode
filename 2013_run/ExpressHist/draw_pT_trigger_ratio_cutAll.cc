...
void draw_pT_trigger_ratio_cutAll()
{
	//Set style
	gROOT->Macro("/afs/cern.ch/user/k/kyolee/private/cms538HI_test/src/UserCode/01_ExpressHist/styleTH1D.C"); 

	//----- open the root file ------
	TFile *openFile = TFile::Open("root://eoscms//eos/cms/store/group/phys_heavyions/velicanu/forest/PA2013_HiForest_Express_r211538_autoforest_v63.root");
...
//		hist1D[i] = new TH1D(histName[i].c_str(),histTitle.c_str(),nBins,xMin,xMax);
	...
	tree->Draw("HLTMuTree.Glb_pt>>All_Global_muons"," HLTMuTree.Event==HltTree.Event && HLTMuTree.Glb_nptl>0 && HLTMuTree.Glb_nTrkFound>10 && HLTMuTree.Glb_glbChi2_ndof<20. && HLTMuTree.Glb_trkChi2_ndof<4. && HLTMuTree.Glb_pixLayerWMeas>0 && TMath::Abs(HLTMuTree.Glb_trkDxy)<3. && TMath::Abs(HLTMuTree.Glb_trkDz)<15. && HLTMuTree.Glb_isArbitrated>0 && TMath::Abs(HLTMuTree.Glb_eta) < 2.4 && (( TMath::Abs(HLTMuTree.Glb_eta)<1.3 && HLTMuTree.Glb_pt>=3.3 ) || ( TMath::Abs(HLTMuTree.Glb_eta)>=1.3 && TMath::Abs(HLTMuTree.Glb_eta)<2.2 && HLTMuTree.Glb_pt*TMath::CosH(Glb_eta)>=2.9 ) || ( TMath::Abs(HLTMuTree.Glb_eta)>=2.2 && HLTMuTree.Glb_pt>=0.8 ))","e");
	tree->Draw("HLTMuTree.Glb_pt>>HLT_PAMu3_v1","HLTMuTree.Event==HltTree.Event && HLTMuTree.Glb_nptl>0 && HLTMuTree.Glb_nTrkFound>10 && HLTMuTree.Glb_glbChi2_ndof<20. && HLTMuTree.Glb_trkChi2_ndof<4. && HLTMuTree.Glb_pixLayerWMeas>0 && TMath::Abs(HLTMuTree.Glb_trkDxy)<3. && TMath::Abs(HLTMuTree.Glb_trkDz)<15. && HLTMuTree.Glb_isArbitrated>0 && TMath::Abs(HLTMuTree.Glb_eta) < 2.4 && (( TMath::Abs(HLTMuTree.Glb_eta)<1.3 && HLTMuTree.Glb_pt>=3.3 ) || ( TMath::Abs(HLTMuTree.Glb_eta)>=1.3 && TMath::Abs(HLTMuTree.Glb_eta)<2.2 && HLTMuTree.Glb_pt*TMath::CosH(Glb_eta)>=2.9) || (TMath::Abs(HLTMuTree.Glb_eta)>=2.2 && HLTMuTree.Glb_pt>=0.8 )) && HltTree.HLT_PAMu3_v1==1 ","pe same");
	tree->Draw("HLTMuTree.Glb_pt>>HLT_PAMu7_v1","HLTMuTree.Event==HltTree.Event && HLTMuTree.Glb_nptl>0 && HLTMuTree.Glb_nTrkFound>10 && HLTMuTree.Glb_glbChi2_ndof<20. && HLTMuTree.Glb_trkChi2_ndof<4. && HLTMuTree.Glb_pixLayerWMeas>0 && TMath::Abs(HLTMuTree.Glb_trkDxy)<3. && TMath::Abs(HLTMuTree.Glb_trkDz)<15. && HLTMuTree.Glb_isArbitrated>0&& TMath::Abs(HLTMuTree.Glb_eta) < 2.4 && (( TMath::Abs(HLTMuTree.Glb_eta)<1.3 && HLTMuTree.Glb_pt>=3.3 ) || ( TMath::Abs(HLTMuTree.Glb_eta)>=1.3 && TMath::Abs(HLTMuTree.Glb_eta)<2.2 && HLTMuTree.Glb_pt*TMath::CosH(Glb_eta)>=2.9) || (TMath::Abs(HLTMuTree.Glb_eta)>2.2 && HLTMuTree.Glb_pt>=0.8 )) && HltTree.HLT_PAMu7_v1==1","pe same");
	tree->Draw("HLTMuTree.Glb_pt>>HLT_PAMu12_v1","HLTMuTree.Event==HltTree.Event && HLTMuTree.Glb_nptl>0 && HLTMuTree.Glb_nTrkFound>10 && HLTMuTree.Glb_glbChi2_ndof<20. && HLTMuTree.Glb_trkChi2_ndof<4. && HLTMuTree.Glb_pixLayerWMeas>0 && TMath::Abs(HLTMuTree.Glb_trkDxy)<3. && TMath::Abs(HLTMuTree.Glb_trkDz)<15. && HLTMuTree.Glb_isArbitrated>0&& TMath::Abs(HLTMuTree.Glb_eta) < 2.4 && (( TMath::Abs(HLTMuTree.Glb_eta)<1.3 && HLTMuTree.Glb_pt>=3.3 ) || ( TMath::Abs(HLTMuTree.Glb_eta)>=1.3 && TMath::Abs(HLTMuTree.Glb_eta)<2.2 && HLTMuTree.Glb_pt*TMath::CosH(Glb_eta)>=2.9) || (TMath::Abs(HLTMuTree.Glb_eta)>2.2&& HLTMuTree.Glb_pt>=0.8 )) && HltTree.HLT_PAMu12_v1==1","pe same");
	//Mu7 as a numerator
...
//	}
	...
	c1->SaveAs(Form("trig_ratio_cutAll_run%d.pdf",Run));
		
	//save as a root file
	TFile *outFile = new TFile(Form("trig_ratio_cutAll_run%d.root",Run),"recreate");
	outFile->cd();
	for (Int_t i=0; i<nHistRatio; i++) {
		histRatio[i]->Write();
	}
