...
void draw_pT_trigger_single_cutAll()
{
	//Set style
	...
	c1->SaveAs(Form("trig_single_cutAll_run%d.pdf",Run));

	//save as a root file
	TFile *outFile = new TFile(Form("trig_single_cutAll_run%d.root",Run),"recreate");
	outFile->cd();
	for (Int_t i=0; i<nHist; i++) {
		hist1D[i]->Write();
	}
